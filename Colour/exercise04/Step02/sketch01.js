...
function draw() {                                   //map(mousex,min,max,min,max)   int(map()) ensures that the result of the map is an integer
  tileCountX = int(map(500, 0, width, 2, 50));      //mapping the mouseX and mouseY to the width and height of the canvas amd
  tileCountY = int(map(400, 0, height, 2, 10));     //assigning the result to the tilecounts to ensure the number of tiles fits within the canvas
